...
//启动一个Worker工作池(开启工作吃的动作只能发生一次，一个zinx框架只能有一个worker工作池)
func (mh *MsgHandle) StartWorkerPool() {
	...
	for i:=0; i < int(mh.WorkerPoolSize); i++ {
		...
		//2 启动当前的Worker，阻塞等待消息从 channel传递进来
		go mh.StartOneWorker(i,mh.TaskQueue[i])
	}
}
//启动一个Worker工作流程
func (mh *MsgHandle) StartOneWorker(workerID int, taskQuue chan ziface.IRequest) {
	fmt.Println("worker ID = ",workerID," is tarted...")

	//不断的阻塞等待对应消息队列的消息
	for {
		select {
			//如果有消息过来，出列的就是一个客户端的Request,执行当前Request所绑定的业务
			case request := <- taskQuue:
...
func (mh *MsgHandle) SendMsgToTaskQueue(request ziface.IRequest) {
	...
	//2 将消息发送给对应的worker的Task Queue即可
